...
export function MultiSelect({ options, selected, onChange, placeholder = 'Select...', allowCustom = false }: Props) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div
        className="min-h-[36px] flex flex-wrap gap-1 px-2.5 py-1.5 rounded-lg border border-gray-200 focus-within:ring-2 focus-within:ring-indigo-400 focus-within:border-transparent cursor-text"
      ...
      {open && (filtered.length > 0 || (allowCustom && query.trim() && !options.includes(query.trim()))) && (
        <div className="dropdown-list absolute z-50 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-36 overflow-y-auto">
          {filtered.map(o => (
            <button
              key={o}
              type="button"
